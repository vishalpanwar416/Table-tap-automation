...
router.get('/login', (req, res) => {
  ...
  const authUrl = `https://www.facebook.com/v20.0/dialog/oauth?client_id=${APP_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${scope}&response_type=code`;
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    try {
      const igUserRes = await axios.get(`https://graph.facebook.com/v20.0/${igAccountId}?fields=username&access_token=${pageAccessToken}`);
